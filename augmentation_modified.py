...
def rotate_image_and_label(img_path, label_path, output_img_path, output_label_path, angle):
    ...
    augmented_img_filename = os.path.join(output_dataset_path, 'images','train', base_filename + '_ro.jpg')
    cv2.imwrite(augmented_img_filename, rotated_img)
    ...
    base_filename = os.path.basename(label_path).split('.')[0]
    augmented_label_filename = os.path.join(output_dataset_path, 'labels','train', base_filename + '_ro.txt')
...
def adjust_brightness(img_path, label_path, output_img_path,output_label_path, angle):
    ...
    base_filename = os.path.basename(img_path).split('.')[0]
    augmented_img_filename = os.path.join(output_dataset_path, 'images','train', base_filename + '_br.jpg')
    cv2.imwrite(augmented_img_filename, adjusted_img)

    base_filename_label = os.path.basename(label_path).split('.')[0]
    augmented_label_filename = os.path.join(output_dataset_path, 'labels', 'train', base_filename_label + '_br.txt')
...
def apply_cropout(img_path, label_path, output_img_path, output_label_path, angle):
    ...
    with open(label_path, 'r') as f:
        ...
        base_filename_label = os.path.basename(label_path).split('.')[0]
    augmented_label_filename = os.path.join(output_dataset_path, 'labels', 'train', base_filename_label + '_cp.txt')
    shutil.copy(label_path, augmented_label_filename)
    ...
    for label in labels:
        ...
        cp_img = img[cutout_y1:cutout_y2, cutout_x1:cutout_x2] = 0
    augmented_img_filename = os.path.join(output_dataset_path, 'images','train', base_filename + '_cp.jpg')
    cv2.imwrite(augmented_img_filename, img)
